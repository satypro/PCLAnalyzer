...
bool FileRead::read(std::string sFilePath, CloudType &cloudData, std::vector <float>&intensity)
{
    bool nErrorCode;
...
bool FileRead::lasFileRead(const std::string sFilePath, CloudType &cloudData, std::vector <float>&intensity)
{
    ...
    for(size_t nIndex = 0; nIndex <_stPubBlk.usnNumOfPoint ; nIndex++) //_stPubBlk.usnNumOfPoint
    {
        ...
        cloudData->points[nIndex].x = (float)lnTempPoint * _stPubBlk.dXScale + _stPubBlk.dXOffset;

         error = fread (&lnTempPoint, 4, 1, fp);
        cloudData->points[nIndex].y = (float)lnTempPoint * _stPubBlk.dYScale + _stPubBlk.dYOffset;

        error = fread (&lnTempPoint, 4, 1, fp);
        cloudData->points[nIndex].z = (float)lnTempPoint * _stPubBlk.dZScale + _stPubBlk.dZOffset;

        error = fread (&usnTempIntensity, 2, 1, fp);
        intensity[nIndex] = (float)usnTempIntensity/255.0;
...
bool FileRead::plyFileRead(const std::string sFilePath,  CloudType &cloudData, std::vector <float>&intensity)
{
  FILE *file;
  char buffer[1000];
...
bool FileRead::readOriginalPtCloud(std::string filename, CloudType &cloudData, std::vector <float>&intensity)
{
  bool errorCode;
...
bool FileRead::offFileRead(std::string filename, CloudType &cloudData, std::vector <float>&intensity)
{
  bool errorCode;
  FILE *fp = fopen(filename.c_str(), "r");
  if(fp == NULL)
...
bool FileRead::xyzFileRead(std::string filename, CloudType &cloudData, std::vector <float>&intensity)
{
  bool errorCode;
  FILE *fp = fopen(filename.c_str(), "r");
  if(fp == NULL)
