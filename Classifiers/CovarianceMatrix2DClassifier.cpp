...
TensorType CovarianceMatrix2DClassifier::GetCoVaraianceTensor(float radius, std::vector<TensorType>& tensors)
{
    ...
    for(int index = 0 ; index < cloud->points.size(); index++)
    {
        ...
        tensors[index].evec0[2] = 0;//covariance_matrix(0, 2);

        tensors[index].evec1[0] = covariance_matrix(1, 0);
        tensors[index].evec1[1] = covariance_matrix(1, 1);
        tensors[index].evec1[2] = 0;//covariance_matrix(1, 2);

        tensors[index].evec2[0] = 0;//covariance_matrix(2, 0);
        tensors[index].evec2[1] = 0;//covariance_matrix(2, 1);
        tensors[index].evec2[2] = 0;//covariance_matrix(2, 2);
    }
}
